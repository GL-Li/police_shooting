...
improve_database <- function(){
    ## add frequently used features including lat, long, and urban-_rural to
    ## database
    ...
    # To save time, do it state by state so that only need to read state census 
    # data for one time
    for (stt in unique(shooting[, state])) {
        shooting[state == stt, urban_rural := identify_area(lat, lon, stt)]
    }  
    
    write.csv(shooting, file = "downloaded_data/database_improved.csv", 
              row.names = FALSE)
}
